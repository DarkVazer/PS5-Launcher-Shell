...
const getGames = () => {
  ...
  return games.map((game) => {
    try {
      ...
      if (game.background && fs.existsSync(game.background)) {
        const ext = path.extname(game.background).slice(1);
        result.background = `data:image/${ext};base64,${fs.readFileSync(game.background).toString('base64')}`;
      } else {
        result.background = '';
      }
      return result;
...
ipcMain.handle('add-game', async (event, game) => {
  ...
  try {
    ...
    if (game.background) {
      const backgroundExt = path.extname(game.background);
      const backgroundName = `${uuidv4()}${backgroundExt}`;
      newGame.background = path.join(gamesDir, backgroundName);
      fs.copyFileSync(game.background, newGame.background);
      console.log(`Copied background to: ${newGame.background}`);
    }
  ...
  if (index >= 0 && index < games.length) {
    try {
      ...
      const updatedGame = {
        ...
        executable: game.executable || '',
        description: game.description || '',
        background: existingGame.background,
      };
      ...
      if (game.background && game.background !== existingGame.background) {
        if (existingGame.background && fs.existsSync(existingGame.background)) {
          fs.unlinkSync(existingGame.background);
        }
        const backgroundExt = path.extname(game.background);
        const backgroundName = `${uuidv4()}${backgroundExt}`;
        updatedGame.background = path.join(gamesDir, backgroundName);
        fs.copyFileSync(game.background, updatedGame.background);
        console.log(`Copied new background to: ${updatedGame.background}`);
      }
...
ipcMain.handle('delete-game', async (event, index) => {
  ...
  if (index >= 0 && index < games.length) {
    ...
    if (game.background && fs.existsSync(game.background)) {
      fs.unlinkSync(game.background);
      console.log(`Deleted background: ${game.background}`);
    }
